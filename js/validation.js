...
function isValidY(y) {
    ...
    if (y !== '' && !isNaN(y) && isFinite(y) && y >= -3 && y <= 3 && activeX && activeR) {
        button.removeAttribute('disabled'); // Валидные данные и активные кнопки X и R, активируем кнопку
        return true;
    } else {
        button.setAttribute('disabled', 'disabled'); // Невалидные данные или отсутствие активных кнопок, делаем кнопку недоступной
        return false;
    }
}
